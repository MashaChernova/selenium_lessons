...
def test_change_currency_in_main_page(browser,base_url):
    ...
    assert cyrrent_symbol in browser.find_element(By.CLASS_NAME,"description").text
...
def test_add_product(browser, base_url):
    ...
    product_page.add_new_product(product_name,"some_tag", "model", "af")
    assert product_name in account_page.find_product(product_name)
...
def test_remove_product(browser, base_url):
    ...
    assert "Success" in account_page.remove_product().text
